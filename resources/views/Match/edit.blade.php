...
@section('content')
    ...
    <div class="d-flex justify-content-around">
        {{Form::open(array('route' => array('match.update', $match->id), 'method' => 'put'))}}
            {{ csrf_field() }}
            ...
            <button class="btn btn-success btn-block my-4" type="submit" id="crearpartido" disabled>{{trans('messages.modificar_partido')}}</button>
    ...
    <script>
        ...
        $(document).on('change',()=>{
            if($('#team1').val()=="" && $('#team2').val()==""){
                ...
                else if(($('#team1').val()!="" && $('#team2').val()!="")&&$('[name="competition"]').val()!="" && fecha.test($('[name="fecha"]').val()) && hora.test($('[name="hora"]').val())){
